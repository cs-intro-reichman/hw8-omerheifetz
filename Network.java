...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        int maxCount = -1;
        for (int i=0; i<this.userCount; i++) {
            if (!this.users[i].equals(toCheck)) {
                if (toCheck.countMutual(this.users[i]) > maxCount) {
                    maxCount = toCheck.countMutual(this.users[i]);
    ...
    public String mostPopularUser() {
        ...
        for (int i=0; i<this.userCount; i++) {
            int currentCheck = followeeCount(this.users[i].getName());
            if (currentCheck > mostPopularAmount) {
                mostPopularAmount = currentCheck;
